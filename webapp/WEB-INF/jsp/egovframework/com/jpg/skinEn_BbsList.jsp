...
<c:if test="${lang == 'en'}">
	<c:set var="sClass" value="1" />
</c:if>

<div id="subWrap"
	class="sub0<c:out value='${sClass}'/> ntc">
				...
				<div class="inner">
					<div id="bo_btn_top">
						...
						<form method="post" class="schForm"
							...
							<input type="text" id="searchWrd" name="searchWrd"
								placeholder="search word" value="${articleVO.searchWrd }"
					...
					<div class="ntc_table">
						<form id="subForm" name="BoardVO" method="get"
							...
							<input class="lang" name="lang" type="hidden" value="">
						...
						<table>
							...
							<tr style="line-height: 58px; border-bottom: 1px solid #333;">
								...
								<th style="width: 20%;" class="disap_68">Writer</th>
								<th style="width: 5%; visibility: visible;" class="disap_68">File</th>
							...
							<c:forEach items="${noticeList}" var="noticeInfo"
								...
								<tr class="bo_notice">
									...
									<td class="articleSub"
										...
										data-skin="<c:out value='${noticeInfo.bbsTyCode}'/>"
										data-lang="<c:out value='${lang}'/>"><input
										type="button" class="goBtn"
										value='<c:out value="${noticeInfo.nttSj}" escapeXml = "false"/>'
										...
										<c:when test="${resultInfo.bbsId == 'BBSMSTR_000000000043'}">
											<td class="articleSub"
												...
												title="<c:out value="${resultInfo.nttSj }"
													...
													<input type="button" class="goBtn" value='Modify'
							...
							<c:if test="${fn:length(resultList) == 0}">
								<tr>
									<td colspan="6" style="text-align:center ;">No Data</td> <!-- 다른 검색 조건 텍스트 수정 위치 -->
								</tr>
							</c:if>
							</tbody>
						</table>
			...
			<div class="btn">
				<form id="writeForm" name="writeForm" method="post"
					...
					<input name="lang" type="hidden"
						value="<c:out value="${lang}"/>">
					<c:if
						test="${user.id == 'webmaster' && boardMasterVO.bbsId != 'BBSMSTR_000000000094'}">
						<input type="button" class="s_submit" id="writeBtn" value="Write"
							title="글쓰기 버튼1">
					</c:if>
					<c:if
						test="${user.id != null && boardMasterVO.bbsId == 'BBSMSTR_000000000094'}">
						<input type="button" class="s_submit" id="writeBtn" value="Write"
							title="글쓰기 버튼2">
					</c:if>
					<c:if
						test="${user == null && boardMasterVO.bbsId == 'BBSMSTR_000000000094'}">
						<input type="button" class="s_submit"
							onclick="javascript:alert('로그인 후 글쓰기가 가능합니다.');" value="Write"
							title="글쓰기 버튼3">
					</c:if>
...
$(".goBtn").on("click", function() {
	...
	$("#subForm .lang").val(baseDom.data("lang"));	
	$("#subForm").submit();
});
