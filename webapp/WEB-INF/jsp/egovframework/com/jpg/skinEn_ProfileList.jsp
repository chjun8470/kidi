...
<div id="subWrap" class="sub01">
	...
	<div id="container">
							...
							<c:forEach var="item" items="${list_menumanage}" varStatus="index">
								...
								<c:if test="${fn:length(item.meCode) == 4 && item.meUse == 1  && item.meLang == lang  }">
									...
									<c:if test="${fn:substring(item.meCode,0,2) == parentCode}">
										...
										<a href="<c:out value="${item.meLink}"/>"><c:out value="${fn:substring(item.meName,0,20)}"/> </a>
										</c:if>
									</c:if> 
							    </c:if>
                			</c:forEach>
        ...
        <div class="contentWrap">
			...
			<c:if test="${meCode == '7040'}">
			<div class="inner">
				<ul class="tab_btn02">
					<li><a href="/contents.do?id=en_par&lang=en">content</a></li>
					<li class="on"><a href="/cop/bbs/selectArticleList.do?bbsId=BBSMSTR_000000000161&lang=en">gallery</a></li>
				</ul>
			</div>
			</c:if>
    ...
    margin-left:55px;
			                ...
			                <div class="list">
			   	                <c:forEach var="item" items="${resultList }" >
				   	                <div>
				                        	...
				                        	<a href="/cop/bbs/selectArticleDetail.do?nttId=${item.nttId}&bbsId=${item.bbsId}&lang=en">
												...
												<div class="<c:if test="${articleVO.bbsId == 'BBSMSTR_000000000141'}">over1</c:if>">
													<div class="fsd_p_box">
														<!-- <p>한줄소개</p> -->
			            ...
			            <c:if test="${user.id == 'webmaster'}">
					        <div class="btn" style="margin-top: 30px;">
								<span class="btn_b">
									<a href="<c:url value='/cop/bbs/insertArticleView.do?bbsId=${boardMasterVO.bbsId}&lang=en' />"  title="<spring:message code="button.create" /> <spring:message code="input.button" />">
